  ...
  */
  ...
  formsubmit: function (res) {
    ...
    that.data.items.push({
      order_num: res.detail.value.order_num
    })
  ...
  clickUp: function () {
    ...
    wx.chooseImage({
      ...
      success: function (res) {
        ...
        // }, 1500)
        // })
        // if (clickUpthis.file) clickUpthis.file(clickUpthis, tempFilePaths, num, clickUpthis.data.i,clickUpthis.file)
        var length = tempFilePaths.length
        clickUpthis.data.picnum = 0
        var i = 0;
        var t1 = setInterval(function () {

          if (i < length) {

            clickUpthis.file(tempFilePaths[i], length);
            i++
          } else {
            clearInterval(t1)
          }
  ...
  file: function (tempFilePath, length) {
    var that = this;
    console.log("picnum=================", that.data.picnum)
    ...
    wx.uploadFile({
      ...
      success: function (res) {
        ...
        if (resData.code == 0) {
          console.log("一张图片上传成功");
          //当进度条为100时清除定时任务
          console.log(that.data.picnum, length)
          if (that.data.picnum == length - 1) {
            wx.hideLoading()
          }
          ...
          // console.log(100 * (that.data.picnum / length))
          that.data.imgPath.push(resData.data.pic_url)
          that.setData({
            progress: Math.ceil(100 * (that.data.picnum / length)),
            imgPath: that.data.imgPath
          })

          // console.log("imgPath", clickUpthis.data.imgPath)
        }
      }
    })
  },
